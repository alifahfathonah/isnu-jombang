<?php
	include ('../config/koneksi.php');
		...
		$x	= md5($password_lama);
		$cek= $connect->query("SELECT * FROM admin WHERE password='$x'");
		...
		if($cek->num_rows){
			...
			if(strlen($password_baru) >= 5){
				...
				if($password_baru == $konfirmasi_password){
					...
					$id_admin 		= $_POST['id_admin']; //ini dari session saat login
					
					$update 		= $connect->query("UPDATE admin SET username='$username', password='$password_baru' WHERE id_admin='$id_admin'");
					if($update){
